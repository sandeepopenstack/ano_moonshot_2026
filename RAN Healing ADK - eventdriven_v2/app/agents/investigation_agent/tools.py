...
def run_investigation_mock(tool_context: ToolContext) -> dict:
    ...
    priority_flag = triage_payload.get("priority_flag", "P1")   

    # ── Scenario selection (can expand later using priority_flag) ─────────
    if domain == "CROSS_DOMAIN":
        scenario = "UC_MULTI_DOMAIN_RCA"
    elif domain == "CORE":
        scenario = "UC2_CORE_CONGESTION"   # HSS Saturation
    else:
        scenario = "UC_SINGLE_RAN"         # UC1 RAN only
