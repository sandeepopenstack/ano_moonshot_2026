"""
app/agents/reflection_agent/agent.py
======================================
ReflectionAgent — 2 tool LlmAgent.

NOTE: This agent is NOT called via LlmAgent.run_async() in root_agent.
root_agent calls its tools directly (deterministic logic, no LLM reasoning needed,
and LLM path caused timeout hangs in loops 5-8).

This agent definition is kept here for:
  - Documentation of the tool contract.
  - Future use if LLM reasoning is needed in reflection.
  - Potential direct invocation via runner in test scenarios.

Tools:
  1. check_execution_result  — parse ExecutorAgent TMF641/TMF921 output
  2. evaluate_and_publish    — re-run GNN, compare z-score, publish result
"""
