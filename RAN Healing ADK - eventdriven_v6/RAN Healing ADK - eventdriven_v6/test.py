...
test.py
=================
Tests the full pipeline WITHOUT any LLM calls.

Proves:
  1. All agent tools work correctly
  2. Pipeline is dynamic — GNN output drives everything
  3. Each agent reads previous agent output correctly
  4. Final result is RESOLVED / IMO_COMPLIES

Step flow:
  2a: EventTriggerMS → /blast-radius  (get affected entities)
  2b: EventTriggerMS → ReflexAgent    (FailureInjectionCreateEvent)
  4a: ReflexAgent    → GNN            (tools call with entity context)
  4b: GNN            → ReflexAgent    (anomalousSubgraph + rankedImpactList + impact_score + criticality_score)
  5:  ReflexAgent    → Detective      (domain triage + priority + entity_ids)
  6:  Detective      → Engineer       (RCA + risk + reversibility)
  7:  Engineer       → Executor       (TMF921 healing plan)
  10: ReflectionAgent                 (post-action z-score check)
"""
import logging
import warnings
import os
...
from failure_injection_ms import (
    get_blast_radius,
    build_trigger_event,
    print_step_2a,
    print_step_2b,
...
from app.agents.reflex_agent.tools import call_gnn_engine, perform_triage, publish_triage
...
class FakeCtx:
    """Minimal ToolContext — no ADK, no LLM needed."""
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 2b: EventTriggerMS → ReflexAgent (FailureInjectionCreateEvent)
# ══════════════════════════════════════════════════════════════════════════════

trigger_payload = build_trigger_event(
    blast_radius,
    event_id="0001",
    event_uuid="EV-dekfn_efjnf_fefe"
)

print_step_2b(trigger_payload)
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 1: ReflexAgent (4a → GNN → 4b → 5)
# ══════════════════════════════════════════════════════════════════════════════
...
print(f"     branch_order   : {reflex_payload.get('execution_order')}")

expected_domain = blast_radius.get("domain", "UNKNOWN")
...
# entity_ids = real synth EIDs from 2b payload (5 for UC1, 1 for UC2, 3 for UC3)
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 2:DetectiveAgent (mock — Ericsson env, Step 6)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 4: ExecutorAgent (mock — Ericsson env, Step 8)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 5: ReflectionAgent (Step 10)
# ══════════════════════════════════════════════════════════════════════════════
...
final_summary = {
    "eventId":          reflection_output.get("eventId"),
