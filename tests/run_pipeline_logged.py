...
def post_agent(url, endpoint, payload, label, timeout=180):
    """POST to agent endpoint and return response JSON."""
    token = get_token()
...
separator("STEP 1: ReflexAgent — POST /trigger_event")
tee(f"\n  Sending 2b FailureInjectionCreateEvent to ReflexAgent...")

reflex_response = post_agent(
    url=SERVICES["reflex"]["url"],
    endpoint="/trigger_event",
    payload={"event": trigger},
    label="ReflexAgent /trigger_event",
)

tee(f"\n  ReflexAgent response:")
tee(json_pp(reflex_response))

# Read actual response fields from server.py triage_result
reflex_payload = {
    "entity_ids":                reflex_response.get("entity_ids", []),
    "anomalous_subgraph":        reflex_response.get("anomalous_subgraph", {}),
    "ranked_list":               reflex_response.get("ranked_list", []),
    "domain_triage":             reflex_response.get("domain_triage"),
    "priority_flag":             reflex_response.get("priority_flag"),
    "priority":                  reflex_response.get("priority"),
    "impact_score":              reflex_response.get("impact_score"),
    "criticality_score":         reflex_response.get("criticality_score"),
    "criticality_label":         reflex_response.get("criticality_label"),
    "reference_time":            reflex_response.get("reference_time"),
    "affected_domains":          reflex_response.get("affected_domains", []),
    "eventId":                   reflex_response.get("eventId"),
    "affected_neighbor_enodebs": trigger.get("affected_neighbor_enodebs", []),
}
...
tee(f"    entity_ids      : {reflex_payload.get('entity_ids')}")
tee(f"    domain_triage   : {reflex_payload.get('domain_triage')}")
tee(f"    priority_flag   : {reflex_payload.get('priority_flag')}")
tee(f"    priority        : {reflex_payload.get('priority')}")
tee(f"    impact_score    : {reflex_payload.get('impact_score')}")
tee(f"    criticality     : {reflex_payload.get('criticality_label')}")
tee(f"    ranked_list     : {[r['node_id'] for r in reflex_payload.get('ranked_list', [])]}")
tee(f"    neighbor_enodebs: {reflex_payload.get('affected_neighbor_enodebs')}")
tee(f"    eventId         : {reflex_payload.get('eventId')}")

calls = ["call_gnn_engine", "perform_triage", "publish_triage"]  # confirmed by server tools
tee("\n  ✅ ReflexAgent PASSED")
...
separator("STEP 3: EngineerAgent — POST /engineer_event")
tee(f"\n  Sending detective.rca.confirmed event to EngineerAgent...")

engineer_response = post_agent(
    url=SERVICES["engineer"]["url"],
    endpoint="/engineer_event",
    payload={"rca_event": rca_event},
    label="EngineerAgent /engineer_event",
)

tee(f"\n  EngineerAgent response:")
tee(json_pp(engineer_response))

engineer_output = engineer_response.get("engineer_output", {})
tmf921         = engineer_output.get("tmf921_intent", {})
...
tee(f"    intent_id       : {engineer_response.get('intent_id')}")
tee(f"    top_utility     : {engineer_response.get('top_utility_score')}")
tee(f"    branch_count    : {engineer_response.get('branch_count')}")
tee(f"    target_entities : {target_entities}")
tee(f"    change_request  : {engineer_output.get('change_request_id')}")
tee(f"    recovery_targets: {[r.get('target_metric') for r in engineer_output.get('recovery_targets', [])]}")

calls_e = ["generate_healing_plan"]  # confirmed by server tools
tee("\n  ✅ EngineerAgent PASSED")

# ── STEP 4: ExecutorAgent ───────────────────────────────────────────────

separator("STEP 4: ExecutorAgent (mock — local)")
exec_output = generate_execution_output(engineer_output)
exec_event  = make_execution_completed_event(
    source_event_id="engineer-ev-1",
...
separator("STEP 5: ReflectionAgent — POST /reflection_event")
tee(f"\n  Sending execution.completed event to ReflectionAgent...")

reflection_response = post_agent(
    url=SERVICES["reflection"]["url"],
    endpoint="/reflection_event",
    payload={"exec_event": exec_event},
    label="ReflectionAgent /reflection_event",
)

tee(f"\n  ReflectionAgent response:")
tee(json_pp(reflection_response))

resolved  = reflection_response.get("resolved", False)
imo_status = reflection_response.get("imo_status")
gui_status = reflection_response.get("gui_dashboard", {}).get("gui_status")

tee(f"\n  Reflection output:")
tee(f"    imo_status      : {imo_status}")
tee(f"    resolved        : {resolved}")
tee(f"    network_status  : {reflection_response.get('network_status')}")
tee(f"    gui_status      : {gui_status}")
tee(f"    business_view   : {reflection_response.get('gui_dashboard', {}).get('business_view')}")
tee(f"    service_view    : {reflection_response.get('gui_dashboard', {}).get('service_view')}")
tee(f"    topology_state  : {reflection_response.get('gui_dashboard', {}).get('topology_state')}")
tee(f"    activation_id   : {reflection_response.get('tmf_metadata', {}).get('activation_id')}")
tee(f"    intent_id       : {reflection_response.get('tmf_metadata', {}).get('intent_id')}")
tee(f"    retrigger_count : {reflection_response.get('retrigger', {}).get('count')}")

calls_r = ["check_execution_result", "evaluate_and_publish"]  # confirmed by server tools
tee("\n  ✅ ReflectionAgent PASSED")

# ── SUMMARY ─────────────────────────────────────────────────────────────

separator("PIPELINE SUMMARY")
tee(f"  STEP 0 TriggerEvent   | PASS")
tee(f"  STEP 1 ReflexAgent    | tools={len(calls)}  | {'PASS' if calls else 'FAIL'}")
tee(f"  STEP 2 DetectiveMock  | PASS")
tee(f"  STEP 3 EngineerAgent  | tools={len(calls_e)} | {'PASS' if calls_e else 'FAIL'}")
tee(f"  STEP 4 ExecutorMock   | PASS")
tee(f"  STEP 5 ReflectionAgent| tools={len(calls_r)} | {'PASS' if calls_r else 'FAIL'}")
tee(f"  RESOLVED              : {resolved}")
tee(f"  IMO STATUS            : {imo_status}")
tee(f"  GUI STATUS            : {gui_status}")
