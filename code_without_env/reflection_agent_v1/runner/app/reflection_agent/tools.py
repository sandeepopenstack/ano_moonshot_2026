...
# ── Gate 2: Anomaly label  [COMMENTED OUT — enable when Spanner is ready] ──────
#
# def _resolve_hex_bins_direct(entity_ids: list[str]) -> list[str]:
#     """
#     Resolve affected EIDs -> h3_07 hex bins via Spanner edge_geobintoentity table.
#     Required because aw_base_hex07_anom is keyed by h3_07, not by eid.
#     """
#     if not entity_ids:
#         return []
#     try:
...
#             ):
#                 if row[0]:
#                     hex_bins.append(row[0])
#         return _dedupe(hex_bins)
#     except Exception as e:
#         logging.warning(
#             f"[ReflectionAgent] Direct hex bin resolution failed: {e} - "
#             f"Gate 2 will fail closed"
#         )
#         return []
#
#
# def _gate2_anomaly_label_direct(h3_hex_bins: list[str]) -> tuple[bool, list[str]]:
#     """
#     Gate 2 (direct Spanner): anomaly_label == "NORMAL" for all affected hex bins.
#     Queries aw_base_hex07_anom, latest snapshot per hex bin.
#     """
#     if not h3_hex_bins:
#         return True, []
#     from google.cloud import spanner as spanner_lib
...
#         for row in snap.execute_sql(
#             """
#             SELECT h3_07, anomaly_label, time_bucket
#             FROM aw_base_hex07_anom
#             WHERE h3_07 IN UNNEST(@bins)
#             ORDER BY time_bucket DESC
#             """,
#             params={"bins": h3_hex_bins},
#             param_types={"bins": arr_type},
#         ):
#             h3, label, _ = row
#             if h3 not in seen:
#                 seen.add(h3)
#                 labels.append((label or "UNKNOWN").upper())
#     gate_ok = bool(labels) and all(label == "NORMAL" for label in labels)
#     return gate_ok, labels
#
#
# def _gate2_anomaly_label_via_mcp(h3_hex_bins: list[str]) -> tuple[bool, list[str]]:
#     if not h3_hex_bins:
#         return True, []
#     rows = _invoke_tool("query_hex_anomaly_labels", {"bins": h3_hex_bins})
#     labels_by_hex = {}
#     for row in rows:
#         h3 = row.get("h3_07")
#         if h3 and h3 not in labels_by_hex:
#             labels_by_hex[h3] = (row.get("anomaly_label") or "UNKNOWN").upper()
#     labels = [labels_by_hex[h3] for h3 in _dedupe(h3_hex_bins) if h3 in labels_by_hex]
#     gate_ok = bool(labels) and all(label == "NORMAL" for label in labels)
#     return gate_ok, labels
#
#
# def _resolve_hex_bins_via_mcp(entity_ids: list[str]) -> list[str]:
#     rows = _invoke_tool("query_entity_hex_bins", {"eids": entity_ids})
#     return _dedupe([
#         row.get("h3_07") or row.get("h3_id")
#         for row in rows
#         if row.get("h3_07") or row.get("h3_id")
#     ])
#
#
# def _resolve_hex_bins(entity_ids: list[str]) -> list[str]:
#     """MCP Toolbox -> direct Spanner fallback for hex bin resolution."""
#     if not entity_ids:
#         return []
#     if _is_toolbox_running():
#         hex_bins = _resolve_hex_bins_via_mcp(entity_ids)
#         logging.info(
#             f"[ReflectionAgent] Hex bins via MCP | "
#             f"entities={entity_ids} | hex_bins={hex_bins}"
#         )
#         return hex_bins
#     logging.warning(
#         f"[ReflectionAgent] MCP Toolbox not running at {_TOOLBOX_URL} - "
#         f"falling back to direct Spanner hex lookup"
#     )
#     return _resolve_hex_bins_direct(entity_ids)
#
#
# def _gate2_anomaly_label(h3_hex_bins: list[str]) -> tuple[bool, list[str]]:
#     """Gate 2 entry point: MCP Toolbox -> direct Spanner fallback."""
#     if not h3_hex_bins:
#         return True, []
#     if _is_toolbox_running():
#         return _gate2_anomaly_label_via_mcp(h3_hex_bins)
#     logging.warning(
#         f"[ReflectionAgent] MCP Toolbox not running at {_TOOLBOX_URL} - "
#         f"falling back to direct Spanner anomaly-label lookup"
#     )
#     return _gate2_anomaly_label_direct(h3_hex_bins)


# ── Gate 3: is_degraded  [COMMENTED OUT — enable when Spanner is ready] ────────
#
# def _gate3_is_degraded_direct(entity_ids: list[str]) -> tuple[bool, list[str]]:
#     """
#     Gate 3 (direct Spanner): is_degraded == False for all affected entities.
#     Queries performance table, latest snapshot per eid.
#     """
#     if not entity_ids:
#         return True, []
#     from google.cloud import spanner as spanner_lib
...
#         for row in snap.execute_sql(
#             """
#             SELECT eid, is_degraded, event_ts
#             FROM performance
...
#                 if _as_bool(is_deg):
#                     degraded.append(eid)
#     missing = [eid for eid in entity_ids if eid not in seen]
#     degraded.extend([f"MISSING_PERFORMANCE:{eid}" for eid in missing])
#     return len(degraded) == 0, degraded
#
#
# def _gate3_is_degraded_via_mcp(entity_ids: list[str]) -> tuple[bool, list[str]]:
#     if not entity_ids:
#         return True, []
#     rows = _invoke_tool("query_entity_performance", {"eids": entity_ids})
#     degraded = []
#     seen = set()
#     for row in rows:
#         eid = row.get("eid")
...
#             continue
#         seen.add(eid)
#         if _as_bool(row.get("is_degraded")):
#             degraded.append(eid)
#     missing = [eid for eid in entity_ids if eid not in seen]
#     degraded.extend([f"MISSING_PERFORMANCE:{eid}" for eid in missing])
#     return len(degraded) == 0, degraded
#
#
# def _gate3_is_degraded(entity_ids: list[str]) -> tuple[bool, list[str]]:
#     """Gate 3 entry point: MCP Toolbox -> direct Spanner fallback."""
#     if not entity_ids:
#         return True, []
#     if _is_toolbox_running():
#         return _gate3_is_degraded_via_mcp(entity_ids)
#     logging.warning(
#         f"[ReflectionAgent] MCP Toolbox not running at {_TOOLBOX_URL} - "
#         f"falling back to direct Spanner performance lookup"
#     )
#     return _gate3_is_degraded_direct(entity_ids)
...
def evaluate_and_publish(tool_context: ToolContext) -> str:
    ...
    # ── Gate 2: Anomaly label restored  [COMMENTED OUT] ──────────────────────
    # Uncomment when Spanner aw_base_hex07_anom is populated post-remediation.
    # Steps to enable:
    #   1. Uncomment Gate 2 functions above (_resolve_hex_bins, _gate2_anomaly_label, etc.)
    #   2. Uncomment the gate2 block below
    #   3. Add `and gate2_ok` to the resolved = line at the bottom
    #
    # gate2_ok = False
    # labels   = []
    # try:
    ...
    #     if not h3_hex_bins:
    #         gate2_ok = False
    #         labels = ["NO_HEX_BINS_RESOLVED"]
    #     elif not labels:
    #         gate2_ok = False
    #         labels = ["NO_ANOMALY_LABELS_FOUND"]
    # except Exception as e:
    #     logging.exception(f"[ReflectionAgent] Gate 2 validation failed: {e}")
    #     h3_hex_bins = []
    #     gate2_ok    = False
    #     labels      = [f"GATE2_FAILED:{str(e)}"]
    #
    # logging.info(
    #     f"[Step 10] Gate 2 — Anomaly Label | "
    #     f"eventId={event_id} | "
    #     f"result={'PASS' if gate2_ok else 'FAIL'} | "
    #     f"anomaly_labels={labels} | "
    #     f"hex_bins={h3_hex_bins}"
    # )

    # ── Gate 3: Entities not degraded  [COMMENTED OUT] ───────────────────────
    # Uncomment when Spanner performance table is populated post-remediation.
    # Steps to enable:
    #   1. Uncomment Gate 3 functions above (_gate3_is_degraded, etc.)
    #   2. Uncomment the gate3 block below
    #   3. Add `and gate3_ok` to the resolved = line at the bottom
    #
    # gate3_ok = False
    # degraded = []
    # try:
    ...
    #     if not target_entities:
    #         gate3_ok = False
    #         degraded = ["NO_TARGET_ENTITIES"]
    # except Exception as e:
    #     logging.exception(f"[ReflectionAgent] Gate 3 validation failed: {e}")
    #     gate3_ok = False
    #     degraded = [f"GATE3_FAILED:{str(e)}"]
    #
    # logging.info(
    #     f"[Step 10] Gate 3 — is_degraded | "
    #     f"eventId={event_id} | "
    #     f"result={'PASS' if gate3_ok else 'FAIL'} | "
    #     f"degraded_entities={degraded}"
    # )
