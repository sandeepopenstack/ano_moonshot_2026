...
def generate_investigation_output(
    ...
    # Add this scenario to generate_investigation_output():
    if scenario == "UC2_CORE_CONGESTION":
        return {
            ...
            "eventTime": datetime.now(timezone.utc).isoformat(),
            "rootCauseAnalysis": {
                "confirmedRootCause": "HSS_SATURATION",
                "domain":             "CORE",
                "severity":           "P1",
                "affectedResources":  ["HSS_CORE_01"],
                "confidence":         0.96,
            },
            "confirmedRcaBranches": [
                {"action_id": "A", "domain": "CORE", "root_cause": "HSS_STALE_SESSION_LOOP", "priority_score": 10},
            ],
            "recommendedHealingScope": "SINGLE_DOMAIN_CORE",
            "businessPriority": "CRITICAL",
        }
