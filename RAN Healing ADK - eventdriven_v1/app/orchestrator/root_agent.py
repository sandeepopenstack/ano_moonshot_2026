...
class RanHealingOrchestrator(BaseAgent):
    ...
    In a future full A2A deployment each tool becomes a
    separate Cloud Run service and this routing becomes
    Pub/Sub subscriptions. The event contracts (events.py)
    stay identical — only the transport layer changes.
    """
    ...
    async def _run_async_impl(
        ...
        if event_type in _ROUTE:
            ...
            if resolved:
                ...
                print(f"\n[Observer] Validation FAILED — retriggering pipeline")
                result = monitor_and_triage(tool_ctx)
