...
def validate_remediation(tool_context: ToolContext) -> dict:
    ...
    # retrigger event includes the full previous GNN payload
    if not resolved:
        # Reuse the original GNN inference so MonitoringAgent can process it
        original_gnn = state.get("latest_gnn_anomaly_detected", {})
        original_payload = original_gnn.get("payload", {
            "retriggered": True,
            "anomalousSubgraph": {"nodes": [], "edges": []},
            "anomalyScore": {"zScore": 9.4, "confidence": 0.97},
            "rankedRemediationBranches": [],
            "businessPriority": "CRITICAL",
        })
        original_payload["retriggered"] = True
        new_gnn_event = make_gnn_anomaly_event(original_payload)
